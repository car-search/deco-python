...
pip install git+ssh://git@github.com/car-search/deco-python.git
```
...
to add `DECO_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncDeco` instead of `Deco` and use `await` with each API call:

```python
import os
import asyncio
from deco import AsyncDeco
...
```sh
# install from the production repo
pip install 'deco[aiohttp] @ git+ssh://git@github.com/car-search/deco-python.git'
```
...
import os
import asyncio
from deco import DefaultAioHttpClient
from deco import AsyncDeco
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `deco.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `deco.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `deco.APIError`.
...
except deco.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except deco.APIStatusError as e:
...
You can enable logging by setting the environment variable `DECO_LOG` to `info`.

```shell
$ export DECO_LOG=info
```
...
```python
import httpx
from deco import Deco, DefaultHttpxClient

client = Deco(
    # Or use the `DECO_BASE_URL` env var
